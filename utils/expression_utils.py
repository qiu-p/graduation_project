...
def optimize_constants(expressions, X_constants, y_constants, inner_lr, inner_num_epochs, optimizer, device):
    ...
    exp_ens = ExpressionEnsemble(expressions_with_constants, device).to(device)
    ...
    y_constants = y_constants.squeeze()
    y_constants_ens = y_constants.repeat(len(expressions_with_constants), 1).to(device)
    ...
    if (optimizer=='lbfgs'):
        ...
        for i in range(inner_num_epochs):
            optim.zero_grad()
            y = exp_ens(Variable(X_constants.to(device), requires_grad=True))
            # set_trace()
            loss = criterion(y, y_constants_ens)
            loss.backward()
            optim.step()
...
class ExpressionEnsemble(torch.nn.Module):
    ...
    def forward(self, x : torch.Tensor) -> torch.Tensor:
        ...
        results = [torch.jit.wait(fut) * torch.ones(x.shape[0]).to(self.device) for fut in futures]
