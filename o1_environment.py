...
class RefineEnv(BaseEnv):
    def __init__(
        ...
        is_multi_obj_condiiton=False, task_index=0, max_target_delay=650, is_sr_model=False, **env_kwargs
    ):
        super().__init__(
            seed, **env_kwargs
        )
        ...
        if self.load_initial_state_pool_npy_path != 'None':
            ...
            self.update_state_pool(state_pool)

    def update_state_pool(self, state_pool):
        for state_dict in state_pool:
            state = state_dict["state"]
            initial_partial_product = PartialProduct[self.bit_width]
            ct32, ct22, partial_products, stage_num = self.decompose_compressor_tree(initial_partial_product[:-1], state)
    ...
    def _call_nn_model(self, ppa_model, ct32, ct22, stage_num):
        ct32 = np.expand_dims(ct32, axis=0)
        ct22 = np.expand_dims(ct22, axis=0)
        if stage_num < self.MAX_STAGE_NUM-1: # self.MAX_STAGE_NUM 设置为4是不是有点小呢？MAX STAGE NUM 应该是用来做图片填充的
            zeros = np.zeros((1, self.MAX_STAGE_NUM-1-stage_num, int(self.int_bit_width*2)))
    ...
    def _call_sr_model(self, ppa_model, ct32, ct22, stage_num):
        sr_feature = []
        # global feature
        ...
        try:
            normalize_area = ppa_model["area_model"].predict(sr_feature)
            normalize_delay = ppa_model["delay_model"].predict(sr_feature)
            print(f"sr model normalize area: {normalize_area}") 
        except:
            sr_feature = torch.tensor(sr_feature, dtype=torch.float).to(self.device)
            with torch.no_grad():
                normalize_area = ppa_model["area_model"](sr_feature)
                normalize_delay = ppa_model["delay_model"](sr_feature)
            normalize_area = normalize_area.item()
            normalize_delay = normalize_delay.item()
            print(f"dsr model normalize area: {normalize_area}") 
        return normalize_area, normalize_delay
    ...
    def get_nextstates(self, state, mask):
        '''
        get next states from a state with every action
        state: np.array (2, int_bit_width*2)
        '''
        next_states = []
        for action_index in range(int(self.int_bit_width*2)*4):
            if not mask[i]:
                next_states.append(None)
                continue
            next_state = copy.deepcopy(state)
            action_column = int(action_index) // 4
            action_type = int(action_index) % 4
            initial_partial_product = PartialProduct[self.bit_width]
            ...
            # 3. The updated final partial product may be invalid, so perform legalization to update the partial product and compressor tree state
            # should be legal, so i delete these code potentialy
            # for i in range(action_column+1, int(self.int_bit_width*2)):
            #     if final_partial_product[i] in [1, 2]:
            #         # it is legal, so break
            #         break 
            #     elif final_partial_product[i] == 3:
            #         # add a 3:2 compressor
            #         next_state[0][i] += 1 
...
if __name__ == '__main__':
    env = RefineEnv(
        1, None, mul_booth_file="mul.test2", bit_width="32_bits_and_gomil",
